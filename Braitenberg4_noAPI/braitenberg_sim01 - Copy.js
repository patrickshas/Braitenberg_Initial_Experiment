...
braitenbergInfo = {
  vehicles: [
        ...
        { x: 330, y: 280, color: "black", angle: 0, baseValues: { left: 3, right: 3, bulb: 0 }, sensors: [{ position: -Math.PI / 4, type: "right", strength: -0.4 }, { position: Math.PI / 4, type: "left", strength: -0.4 }] },
        ...
        { x: 370, y: 320, color: "cyan", angle: 0, baseValues: { left: 1, right: -1, bulb: 0 }, sensors: [{ position: -Math.PI / 4, type: "right", strength: -0.3 }, { position: Math.PI / 4, type: "left", strength: 3 }] },
        ...
        { x: 150, y: 150, color: "aquamarine", angle: 0, baseValues: { left: 2, right: 1, bulb: 0 }, sensors: [{ position: -Math.PI / 4, type: "right", strength: 0.5 }, { position: Math.PI / 4, type: "left", strength: 0.5 }] },
    ...
    bulbs: [{ x: 0, y: 0 }, { x: 312.5, y: 200 }, { x: 200, y: 300 }, { x: 200, y: 200 }, { x: 250, y: 250 }]
};
...
function makeVehicle(newValue) {
  ...
  vehicle.breed = newValue+1; // this sets the breeds
...
function makeVehicle01(newValue) {
    n = newValue;
    ...
    vehicle.onDragging = vehicleFunctions.onDragging;
    vehicle.onDrop = vehicleFunctions.onDrop;
    //mutVal = Math.random() * 0.02;
    mutVal = 0.99 + (Math.random() * 0.02);
    vehicle.vehicle.sensors[0].strength = vehicle.vehicle.sensors[0].strength * mutVal;
    vehicle.vehicle.sensors[1].strength = vehicle.vehicle.sensors[1].strength * mutVal;
    return vehicle;
}
...
function mutateVehicle(vehicle) {
  ...
  vehicles[i] = null;
  vehicles[i] = makeVehicle01(vehicles[n].breed - 1);

  


  //vehicles[i].vehicle.sensors[0].strength = vehicle.vehicle.sensors[0].strength;
  //vehicles[i].vehicle.sensors[1].strength = vehicle.vehicle.sensors[1].strength;
  //braitenbergInfo.vehicles[i].sensors[0].strength = vehicle.vehicle.sensors[0].strength;
  //braitenbergInfo.vehicles[i].sensors[1].strength = vehicle.vehicle.sensors[1].strength;
...
function overlap(vehicle){
  ...
  for(i = 0; i< vehicles.length; i++){
    if(vehicles[i] != vehicle){
      // code here for overlap if needed
    }
  }
...
function replaceDead(vehicle){
  ...
  // Logic to get rid of vehicles now
  

  //mx = simulationInfo.height - simulationInfo.vehicleSize*2 - 1;
  //my = simulationInfo.height - simulationInfo.vehicleSize*2 - 1;
  
  /*do{
    // Random placement
    //vehicle = makeInteractiveElement(new VehicleGraphics(newVehicle), document.getElementById("arenaBraitenberg"));
    nX = Math.floor(Math.random() *mx);
    nY = Math.floor(Math.random() * my);
    vehicle.x = nX;
    vehicle.Y = nY;
  } while(overlap(newVehicle));
  */
   

}
/*
function createVehicle(newValue) {
  n = newValue;
  vehicles[n] = makeInteractiveElement(new VehicleGraphics(//one of the other vehicles with variation//.vehicles[n]), document.getElementById("arenaBraitenberg"));
